...
func TestApplyOutputCaps_NoKnobs(t *testing.T) {
	...
	in := &GenerationOptions{MaxTokens: i32p(99999)}
...
func TestApplyOutputCaps_HardClampDown(t *testing.T) {
	...
	in := &GenerationOptions{MaxTokens: i32p(5000)}
...
func TestApplyOutputCaps_HardCapAppliedWhenUnset(t *testing.T) {
	...
	in := &GenerationOptions{} // MaxTokens nil
	out := ApplyOutputCaps(in)
	if out == nil || out.MaxTokens == nil {
...
func TestApplyOutputCaps_HardCapDoesNotRaiseSmallerValue(t *testing.T) {
	...
	in := &GenerationOptions{MaxTokens: i32p(200)}
	out := ApplyOutputCaps(in)
	if out == nil || out.MaxTokens == nil {
...
func TestApplyOutputCaps_HardAndDefault(t *testing.T) {
	...
	cases := []struct {
		...
		{"unset → default", &GenerationOptions{}, 800},
		{"under hard, kept", &GenerationOptions{MaxTokens: i32p(500)}, 500},
		{"above hard, clamped", &GenerationOptions{MaxTokens: i32p(9999)}, 1000},
		{"equal to hard, kept", &GenerationOptions{MaxTokens: i32p(1000)}, 1000},
	}
	for _, c := range cases {
...
// copies all other GenerationOptions fields when it needs to allocate a new
